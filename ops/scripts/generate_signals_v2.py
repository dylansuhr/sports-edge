    ...
    def calculate_fair_probability(
        ...
        try:
            ...
            if market_name == 'moneyline':
                ...
                # NFL average total is ~45 points, std dev ~10
                ...
                if 'Over' in selection:
                    # P(total > line)
                    over_prob = float(1 - stats.norm.cdf(line_value, loc=league_avg_total, scale=league_std))
                    return over_prob
                elif 'Under' in selection:
                    # P(total < line)
                    under_prob = float(stats.norm.cdf(line_value, loc=league_avg_total, scale=league_std))
                    return under_prob
                else:
